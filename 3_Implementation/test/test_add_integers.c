...
void automated_test_add_integers()
{
  ...
  TEST_ASSERT_EQUAL(SUCCESS, add_integers(100,35, &result_sum));
  TEST_ASSERT_EQUAL(135,result_sum);

  TEST_ASSERT_EQUAL(SUCCESS, add_integers(-10,-30, &result_sum));
...
void automated_test_add_underflow()
{
  int expected_sum = 0;
  int result_sum= 0;
  TEST_ASSERT_EQUAL(OVERFLOW, add_integers(INT_MIN,-1,&result_sum));
